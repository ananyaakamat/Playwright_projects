...
async function createExcelFile(data) {
    try {
        // Create Excel-like CSV file since we don't have Excel library
        const csvHeaders = 'First Name,Last Name,Email,Phone,Address,City,State,ZIP Code,Country,Password\n';
        const csvData = `${data.firstName},${data.lastName},${data.email},${data.phone},${data.address},${data.city},${data.state},${data.zipCode},${data.country},${data.password}`;
...
async function registerUser(page) {
    ...
    try {
        ...
        await page.fill('#email_address', testData.email);
        console.log(`✓ Email: ${testData.email}`);
        
        await page.fill('#password', testData.password);
        console.log('✓ Password entered');
        
        await page.fill('#password-confirmation', testData.confirmPassword);
        ...
        try {
            await page.waitForFunction(() => {
                const spinner = document.querySelector('.loading-mask, .loader, .spinner, [class*="load"]');
        ...
        // Wait for final page stabilization before screenshot
        console.log('Ensuring page is fully rendered before screenshot...');
...
async function recoverPassword(page) {
    ...
    try {
        ...
        await page.goto('https://magento.softwaretestingboard.com/customer/account/forgotpassword/');
        ...
        await page.click('button[title="Reset My Password"]');
...
async function runCompleteFlow() {
    ...
    try {
        ...
        console.log('   Note: [timestamp] format: 19July2025_12_33_PM');

        // Clean up old screenshots after new ones are created
